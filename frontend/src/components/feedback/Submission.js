...
export default function Submission() {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        const response = await fetch(`http://localhost:5000/backend/user_reviews/647fb59f731f38f61bef8036`, {
          method: 'GET',
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://localhost:5000/backend/user_reviews/${customerinfo.user._id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${cookies.accessToken}`,
                ...
                <MDBCard className="text-center">
                  <MDBCardBody>
                    ...
                    <div className="d-flex justify-content-center mt-4">
                      ...
                      <Link to="/feedback" className="btn btn-secondary" onClick={handleDelete}>
                        Delete
                      </Link>
                    </div>
                  </MDBCardBody>
